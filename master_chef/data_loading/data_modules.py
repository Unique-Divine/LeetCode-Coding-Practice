...
class ToyClassificationDM(pl.LightningDataModule):
...
class ToyRegressionDM(pl.LightningDataModule):
    def __init__(self, batch_size: int):
        super().__init__()
        self.batch_size = batch_size

    def prepare_data(self):
        toy_diabetes: TabularDataset = toy_datasets.SklearnToys.diabetes()
        n_samples = toy_diabetes.n_samples
        train_size = round(0.9 * n_samples)
        self.train_val_size = train_size
        test_size = n_samples - train_size
        self.train_data, self.test_data = dataset.random_split(
            dataset=toy_diabetes, lengths=[train_size, test_size])
